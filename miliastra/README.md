...
★★ **建控件的顺序 = 图层顺序**（真机实测，2026-09-25）：**后建的控件盖在上面**，
没有单独的 z 设置。所以装饰控件（描边/光晕/环）必须先建、**文字最后建**。
真机上"核糖体两颗待发球看不到字母"就是我把描边建在了字母之后 —— 白圈压住了字。
⚠ 本地试玩台的渲染器**按控件类型排序**，看不出这个 z 序问题：**改绘制顺序必须靠真机或人工核对**。

★★ **光标常驻 + CursorEvent 的前置条件**（官方 API §24 `ClientUIContainerControl`）：
`showCursor`（编辑器里叫【显示常驻光标】）是**读写**字段，原文写着
「**CursorEvent 相关方法都需设置该参数为真后才可正常使用**」。
不设它，玩家得**按住 Alt** 才看得见光标（真机实测）。脚本启动时会自动
`root.showCursor = true`（挂载点就是容器节点），不用手动勾。

★★ **动态创建的图片控件不会继承模板图**（2026-09-25 真机实测）：必须脚本自己
`SetImage(Enum.ImageSource.StaticReference, 资产号)`，否则每个图片控件都画成"?" ——
球是一个个问号、轨道是一堆问号拼的、中央一个大问号，全是这个原因。
⚠ 两个数字别搞混：**控件模板索引**（1073741852 这种）≠ **资产号**（编辑器里那张图的 id，100002）。
脚本默认用资产号 100002（一张白圆图），可用 `artImage=<资产号>` 换、或 `art=A:id,U:id,…` 分别指定；
启动时也会先在画布里找一张配好图的图片控件"借"它的资产号。
...
| 本地能跑一整关 | ✅ | 假宿主 **373 项断言**（含打包产物端到端） |
| 在客户端 Lua 运行时里真跑 | ✅ | 试玩台：740 个控件建出来、整关推进、可出图 |
| 打包产物可直接上传 | ✅ ~117 KB，14 模块 | `out/zuma.lua`（219,538 字节） |
...
- 彩蛋「璃月黄金交易所」的玩法规则来自玩家自制的千星奇域作品《璃月黄金交易所》，
  经原作者授权移植，**授权条件为注明来源** —— 因此本项目（含 zuma.lua / zuma-pc.zip）
  **必须保留**以下署名：

> **感谢傻傻白老师对本项目的支持**

  署名位置与保留要求见 docs/07-第三方致谢与授权.md（三处：egg.lua 文件头 / 彩蛋面板 / 本文件）。
